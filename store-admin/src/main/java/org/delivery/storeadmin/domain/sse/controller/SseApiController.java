...
public class SseApiController {
    ...
    public ResponseBodyEmitter connect(
        ...
        var emitter = new SseEmitter(1000L * 60 ); // ms timeout
        userConnection.put(userSession.getUserId().toString(), emitter);

        emitter.onTimeout(() -> {
            log.info("on timeout user {}", userSession.getUserId());
            // 클라이언트와 타임아웃이 일어났을때
            emitter.complete();
        });

        emitter.onCompletion(() -> {
            log.info("on completion user {}", userSession.getUserId());
